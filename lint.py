...
def simulate_program(program):
    ...
    while ip < len(program):
        ...
        if op[0] == OP_PUSH:
            ...
            assert len(op) >= 2, "`else` instruction does not have a referene to the end of its block. Please call crossreference_blocks() on the program before trying to simulate."
            ip = op[1]
        elif op[0] == OP_END:
            assert len(op) >= 2, "`end` instruction does not have a referene to the next instruction to jump to. Please call crossreference_blocks() on the program before trying to simulate."
            ip = op[1]
        elif op[0] == OP_DUMP:
            a = stack.pop()
            print(a)
    ...
    with open(out_file_path, "w") as out:
        ...
        for ip in range(len(program)):
            ...
            if op[0] == OP_PUSH:
                ...
                out.write("    ;; -- else --\n")
                assert len(op) == 2, "`else` instruction does not have a referene to the end of its block. Please call crossreference_blocks() on the program before trying to compile it."
                out.write("    jmp addr_%d\n" % op[1])
            elif op[0] == OP_END:
                assert len(op) >= 2, "`end` instruction does not have a referene to the next instruction to jump to. Please call crossreference_blocks() on the program before trying to compile."
                out.write("    ;; -- end -- \n")
                if not ip + 1 == op[1]:
                    out.write("    jmp addr_%d\n" % op[1])
            elif op[0] == OP_DUP:
                ...
                out.write("    pop rax\n")
                out.write("    test rax, rax\n")
                assert len(op) == 2, "`do` instruction does not have a referene to the end of its block. Please call crossreference_blocks() on the program before trying to compile it."
                out.write("    jz addr_%d\n" % op[1])
            else:
                assert False, "Unreachable"
...
def crossreference_blocks(program):
    ...
    for ip in range(len(program)):
        op = program[ip]
        assert COUNT_OPS == 12, "Exhaustive handling of ops in crossreference_blocks. Keep in mind that not all of the ops need to be handled here. Only those that form blocks."
        if op[0] == OP_IF:
            stack.append(ip)
        elif op[0] == OP_ELSE:
            if_ip = stack.pop()
